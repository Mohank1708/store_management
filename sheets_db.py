...
def add_item(item_name, category, quantity, unit, username, user_id=None, rate=0, amount=None, vendor='', notes=''):
    """Add item to inventory (purchase)"""
    try:
        print(f"[DEBUG] Adding item: {item_name}, qty: {quantity}, unit: {unit}")
        spreadsheet = get_spreadsheet()
        inventory_sheet = spreadsheet.worksheet('inventory')
        transactions_sheet = spreadsheet.worksheet('transactions')
        
        timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        # Use amount if provided, otherwise calculate from rate
        total_amount = float(amount) if amount else (float(quantity) * float(rate) if rate else 0)
        ...
        for idx, item in enumerate(items, start=2):
            if item['item_name'].lower() == item_name.lower():
                ...
                item_found = True
                print(f"[DEBUG] Updated existing item: {item_name}, new qty: {new_quantity}")
                break
        ...
        transactions_sheet.append_row([
            ...
            timestamp, notes, float(rate) if rate else 0, total_amount, vendor if vendor else ''
        ])
        print(f"[DEBUG] Transaction logged for: {item_name}")
        
        return True
    except Exception as e:
        print(f"[ERROR] Error adding item: {e}")
        import traceback
        traceback.print_exc()
        return False
